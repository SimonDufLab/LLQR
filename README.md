...
- public eval surface: batched beam-search generation with `beam_size=5`, fairseq-compatible padded-source-width `max_len=1.2*src_len+10`, PAD suppression, `min_len=1` step-0 EOS suppression, top-`2 * beam_size` candidate handling, forced final EOS, length-normalized finalized scoring, `@@ ` BPE removal, Moses detokenization, sacreBLEU scoring, sampled periodic BLEU logged as `valid bleu_sampled`, and full BLEU-routed best-checkpoint snapshots when `preempt_handling=true`
