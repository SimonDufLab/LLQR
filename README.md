...
The dataset surface now also includes the fairseq-faithful local `iwslt14_de_en` text contract under `configs/dataset/iwslt14_de_en.yaml`; it expects an extracted `iwslt14.tokenized.de-en/` directory, writes a local `.llqr_numeric_cache/`, maps the current `run.py` held-out eval path to `valid`, and exposes deterministic `valid` and `test` split helpers for bounded generation and BLEU checks. The train loader now defaults to compile-stable percentile bucketing (`dataset.train_shape_bucket_mode=percentile`, `dataset.train_shape_bucket_count=8`) so JAX sees a bounded set of fairseq-like token-budget batch shapes instead of recompiling on every new seq2seq batch shape.
