...
- model defaults: fairseq-style 6 encoder layers, 6 decoder layers, embed dim `512`, FFN dim `1024`, heads `4`, `relu`, pad-aware sinusoidal positions, and tied decoder input/output embeddings
...
- public eval surface: token validation logs `valid loss` in nats/token and `valid_nll_loss` in fairseq-comparable bits/token; batched beam-search generation uses `beam_size=5`, fairseq-compatible padded-source-width `max_len=1.2*src_len+10`, PAD suppression, `min_len=1` step-0 EOS suppression, top-`2 * beam_size` candidate handling, forced final EOS, length-normalized finalized scoring, `@@ ` BPE removal, Moses detokenization, sacreBLEU scoring, sampled periodic BLEU logged as `valid bleu_sampled`, and full BLEU-routed best-checkpoint snapshots when `preempt_handling=true`
